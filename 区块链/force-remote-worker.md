...
### worker重启后，删除之前的所有layer文件
worker 每次重启， 都会把之间的数据文件删除，比如之前做了一半的， 或者做到90%的， 都会删除， 这个叫重置。 
worker id 的机制有争议， 暂忽略

### p2 内存
p2. 平时是10个G， 但最后快结束的时候，是88个G。 
### 32G的sector空间缓存大小
...
p3 做完后， p4和 copytask会同时做，p3会对p2做验证， p3验证后， copytask直接把封装好的sealed数据文件和cache下的一些小文件拷贝到最终目录下： 
...
33G	total
```
所以一个32G的sector 占用cache+seal = 420G空间


copytask是紧随p3完成后。立即做的。 所以16的任务会很快就看到结束， 而8的任务还在进行中。 

cleantask要等到sector进入Proving 之后，才会删除， 所以32的任务看到，就表明sector已经proving了。
...
#### p2生成sealed文件
p2除了生成tree-r-last, tree-c 文件， 还要生成一个sealed 文件， 这个文件是最终封装的文件， 是要被copytask拷贝的文件， 在force work log会有这个sealed文件名:
![-w1510](media/15952298123241.jpg)
...
11台机器写文件 ， 证明需要这些文件 
sealer 按机会分组， 一个组起一个sealer, 分到机器，做任务
...
#### p1 cache的落盘与计算
p1 阶段要使用的cache文件，  算出来， 看有没落盘， 然后从磁盘上读出来。  让他去读。

```
[root@instance-20200716-0836 ~]# screen -r
There is a screen on:
        3683.pts-0.instance-20200716-0836       (Attached)
There is no screen to be resumed.
您在 /var/spool/mail/root 中有新邮件

[root@instance-20200716-0836 ~]# screen -r 3683.pts-0.instance-20200716-0836
There is a screen on:
        3683.pts-0.instance-20200716-0836       (Attached)
There is no screen to be resumed matching 3683.pts-0.instance-20200716-0836.
```