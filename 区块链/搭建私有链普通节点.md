...
./lotus send t3地址(lotus的t3地址) 0.00001
```
...
初始化前需要将.lotusstorage 目录删除， 如果不是和lotus进程在同一台主机， 需要将lotus进程所在主机的.lotus/api, ./lotus/token覆盖掉本机同目录下文件。
...
### （九）启动poster， sealer,  force-remote-worker的步骤相同和 《搭建公链节点》相同
