...
然后运行 
```
./lotus-storage-miner init --nosync --sector-size=536870912 
```
...
假如lotus , 坏了， 两台, vip飘到了B
vip票过，但tcp链接有问题， tcp是假链接， 只有重启， 

重练后， ip地址可以不变， 包括.lotus不变， 
...
health-check, 谁发生了发生， 去三个链的高度， 判读优美有分叉 
...
防火墙贵的原因 ：
就解决session, 同步， 在客户段没有感觉下， 把链接建立到另一个链接。 
...
---
lotus-server 负载均衡， 在k8s里
nginx , 发送大龄的请求， 看lotus-sever成熟的压力怎么样，兵法写数据
...
---
lotus-message 多个节点操作数据库， 大批量请求， 有没有锁表， 
成功率
