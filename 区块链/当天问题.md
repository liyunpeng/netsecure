...
怎么保证worker不跑到别的机会下， 配置文件写错了， 怎么能自检， 

###  sealer启动命令的dist-path的意义

p4 完成后， p5就会zh


###  服务进程应减少对环境变量的依赖，自己去读取配置生成

从config.toml 里读到32G, 
程序自己读到系统配置， 比如大于128G， 128G 到 192 G之间， 

程序可以自身设置 参数， 


能不依赖环境变量的就不依赖， 让程序自己根据配置文件， 和系统信息 初始化好程序的参数。 

一个机会集群由一台机器完全控制。 
是否现在可以做这个检测面板。 

登陆到集群里一台主机， 提供一个web面板。 
